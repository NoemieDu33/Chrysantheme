...
class SheetTab:
    ...
    def build_ui(self):
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                if i == 0 and j == 0:
                    # --- CELLULE (0,0) : COMPOSANT TEXT SÉCURISÉ ---
                    # grid_propagate(False) empêche le Text de gonfler la grille à sa convenance
                    ...
                    t.bind("<BackSpace>", lambda event, widget=t: self.force_space_on_delete(widget, is_text=True))
                    t.bind("<Delete>", lambda event, widget=t: self.force_space_on_delete(widget, is_text=True))
                else:
                    e = tk.Entry(
                        self.frame, 
                        width=15, 
                    ...
                    # Assigner la valeur par défaut " " (un espace)
                    e.insert(0, "-")
                    
                    self.grid_entries[(i, j)] = e
                    
                    # Détection des touches pour les couleurs
                    e.bind("<KeyRelease>", lambda event, entry=e: self.check_cell_color(entry))
                    
                    # Sécurité pour forcer l'espace lors d'une suppression
                    e.bind("<BackSpace>", lambda event, entry=e: self.force_space_on_delete(entry))
                    e.bind("<Delete>", lambda event, entry=e: self.force_space_on_delete(entry))

    def force_space_on_delete(self, entry):
        """
        S'exécute juste après qu'une touche de suppression soit pressée.
        Si l'utilisateur efface tout, on remet l'espace obligatoire.
        """
        # after_idle permet d'attendre que Tkinter applique l'effet du Backspace/Delete avant de vérifier
    ...
    def check_cell_color(self, entry):
        ...
        if val[0] in ["1","2","3","4","5","6","7","8","9","C","F","J","D","E","I"]:
            ...
            entry.config(bg=self.bgcolor, fg=self.fgcolor) # Retour au thème par défaut
    ...
    def get_content(self):
        ...
        for i in range(self.rows):
            ...
            for j in range(self.columns):
                ...
                # Si par mégarde c'est vide, on sauvegarde un espace
                if not val:
                    val = " "
    ...
    def set_content(self, content_str):
        ...
        for i, line in enumerate(lines):
            ...
            for j, cell_value in enumerate(cells):
                if j >= self.columns: break
                entry = self.grid_entries[(i, j)]
                entry.delete(0, tk.END)
                # Si la cellule lue est vide, on force l'espace
                if cell_value == "":
                    cell_value = " "
...
class NotesApp:
    ...
    def new_roundstart(self):
        ...
        if not char:
            return # Annulation si aucune sélection

        path = Path(f"profiles/{self.pseudo}/{self.perso}/roundstart/{char}.pipi")
        
        # Création et ajout de l'onglet Grille
        ...
        # Si le fichier .pipi existe déjà sur le disque, on le charge
        if path.exists():
            try:
                content = path.read_text(encoding="utf-8")
    ...
    def save_file(self):
        ...
        if not tab.filepath:
            # On adapte l'extension par défaut selon le type de l'onglet
        ...
        # On extrait le contenu selon le type d'onglet
